...
拿状态转移方程 f(n)= f(n-1) + f(n-2) 来说。我们要计算f(20)需要计算f(19)和f(18)，计算f(19)需要计算f(18)和f(17)，由此可见每个子问题会计算两次。为了避免重复计算需要引入一个备忘录。
...
动态规划问题最困难的就是写出状态转移方程，你把 f(n) 想做一个状态 n，这个状态 n 是由状态 n - 1 和状态 n - 2 相加转移而来，这就叫状态转移。
